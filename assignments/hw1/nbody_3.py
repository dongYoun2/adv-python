...
def advance(dt):
    ...
    '''
    bodies = BODIES
    f_compute_deltas = compute_deltas
    f_update_vs = update_vs
    f_update_rs = update_rs
    ...
    for body1 in bodies.keys():
        for body2 in bodies.keys():
            if (body1 != body2) and not (body2 in seenit):
                ...
                (dx, dy, dz) = f_compute_deltas(x1, x2, y1, y2, z1, z2)
                f_update_vs(v1, v2, dt, dx, dy, dz, m1, m2)
                seenit.append(body1)

    for body in bodies.keys():
        (r, [vx, vy, vz], m) = bodies[body]
        f_update_rs(r, dt, vx, vy, vz)
...
def report_energy(e=0.0):
    ...
    bodies = BODIES
    f_compute_deltas = compute_deltas
    f_compute_energy = compute_energy
    ...
    for body1 in bodies.keys():
        for body2 in bodies.keys():
            if (body1 != body2) and not (body2 in seenit):
                ...
                ((x2, y2, z2), v2, m2) = bodies[body2]
                (dx, dy, dz) = f_compute_deltas(x1, x2, y1, y2, z1, z2)
                e -= f_compute_energy(m1, m2, dx, dy, dz)
                seenit.append(body1)
